...
@interface HTMLFiveWebViewController () <WKUIDelegate, WKNavigationDelegate, CustomWkWebViewDelegate, UINavigationBarDelegate, UIImagePickerControllerDelegate>
...
- (CustomWkWebView *)wkWebView {
    if (!_wkWebView) {
        ...
        if (_whetherShowTabbar == canShowTabbar) {//显示底部工具栏
            frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height-TAB_BAR_HEIGHT);
        } else {
